    ...
    public class _LL41_25_26_DeriveAndWrapKeyTest
    {
        ...
        public void _LL41_25_26_01_DeriveAndWrap_VKO_Gost3410_01_Test()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                rv = Helpers.GenerateGostKeyPair(pkcs11, session, ref senderPubKeyId, ref senderPrivKeyId, Settings.GostKeyPairId1);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = Helpers.GenerateGostKeyPair(pkcs11, session, ref recipientPubKeyId, ref recipientPrivKeyId, Settings.GostKeyPairId2);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());

                // Выработка общего ключа на стороне отправителя
                uint senderDerivedKeyId = CK.CK_INVALID_HANDLE;
                rv = Helpers.Derive_GostR3410_Key(pkcs11, session, recipientPubKeyId, senderPrivKeyId, ukm, ref senderDerivedKeyId);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                rv = Helpers.Derive_GostR3410_Key(pkcs11, session, senderPubKeyId, recipientPrivKeyId, ukm, ref recipientDerivedKeyId);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());

                Assert.IsTrue(recipientDerivedKeyId != CK.CK_INVALID_HANDLE);
        ...
        public void _LL41_25_26_02_DeriveAndWrap_VKO_Gost3410_12_Test()
        {
            ...
            using (Pkcs11 pkcs11 = new Pkcs11(Settings.Pkcs11LibraryPath))
            {
                ...
                uint senderPubKeyId = CK.CK_INVALID_HANDLE;
                uint senderPrivKeyId = CK.CK_INVALID_HANDLE;
                rv = Helpers.GenerateGost512KeyPair(pkcs11, session, ref senderPubKeyId, ref senderPrivKeyId, Settings.Gost512KeyPairId1);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                uint recipientPubKeyId = CK.CK_INVALID_HANDLE;
                uint recipientPrivKeyId = CK.CK_INVALID_HANDLE;
                rv = Helpers.GenerateGost512KeyPair(pkcs11, session, ref recipientPubKeyId, ref recipientPrivKeyId, Settings.Gost512KeyPairId2);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());

                // Выработка общего ключа на стороне отправителя
                uint senderDerivedKeyId = CK.CK_INVALID_HANDLE;
                rv = Helpers.Derive_GostR3410_12_Key(pkcs11, session, recipientPubKeyId, senderPrivKeyId, ukm, ref senderDerivedKeyId);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
                ...
                // Выработка общего ключа на стороне получателя
                uint recipientDerivedKeyId = CK.CK_INVALID_HANDLE;
                rv = Helpers.Derive_GostR3410_12_Key(pkcs11, session, senderPubKeyId, recipientPrivKeyId, ukm, ref recipientDerivedKeyId);
                if (rv != CKR.CKR_OK)
                    Assert.Fail(rv.ToString());
